    ...
    def extractpypi(self,packagelist=[], sourceIndex='/mnt/DATA/projects/bandersnatch/mirror/web/simple/', sourcePackages='/mnt/DATA/projects/bandersnatch/mirror/web/packages/', dest='/mnt/DATA/temptemp/pypi6', ssh="jax/Dh123/jax79sg.hopto.org/10022"):
        ...
        with tqdm.tqdm(total=len(packagelist)) as pbar:
            for package in packagelist:
                listOfPackages=self.processIndexHtmls(package, sourceIndex, dest, ssh=ssh)
                self.processPackages(listOfPackages,sourcePackages, dest, ssh=ssh)
                pbar.update()
        self.scp.close()
...
ptm=pypitomirror()
ptm.extractpypi(['transformers','tokenizers'])
